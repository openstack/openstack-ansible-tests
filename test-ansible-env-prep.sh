...
if [[ -d "${OSA_INTEGRATED_REPO_HOME}" ]]; then
  ...
  PIP_OPTS+=" --constraint https://github.com/openstack/openstack-ansible/raw/${TESTING_BRANCH}/global-requirement-pins.txt"
fi
...
if [[ -d "${REQS_REPO_HOME}" ]]; then
  ...
  PIP_OPTS+=" --constraint ${UPPER_CONSTRAINTS_FILE:-https://github.com/openstack/requirements/raw/${TESTING_BRANCH}/upper-constraints.txt}"
fi
